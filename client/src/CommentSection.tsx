    ...
    <section className="space-y-4">
      <div className="space-y-3">
        {comments.data?.length ? comments.data.map((comment) => {
          ...
          return <article key={comment.id} className="rounded-xl border p-4 card"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold">{comment.author?.name ?? 'Team member'}</p><p className="text-xs muted">{displayTime(comment.createdAt)}</p></div>{canManage && <div className="flex gap-1"><button aria-label="Edit comment" type="button" onClick={() => { setEditing(comment); form.reset({ content: comment.content }); }} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100"><Pencil size={15} /></button><button aria-label="Delete comment" type="button" onClick={() => { if (window.confirm('Delete this comment? This action cannot be undone.')) void remove.mutateAsync(comment.id); }} className="rounded-lg p-2 text-slate-500 hover:bg-red-50 hover:text-red-600"><Trash2 size={15} /></button></div>}</div><p className="mt-3 whitespace-pre-wrap text-sm muted">{comment.content}</p></article>;
      ...
      <form onSubmit={submit} className="rounded-xl border p-4 card"><div className="flex items-center justify-between"><h3 className="text-sm font-semibold">{editing ? 'Edit comment' : 'Add a comment'}</h3>{editing && <button type="button" onClick={() => { setEditing(null); form.reset({ content: '' }); }} className="inline-flex items-center gap-1 text-xs muted"><X size={14} /> Cancel</button>}</div><textarea {...form.register('content')} rows={4} placeholder="Write a note for the team…" className="mt-3 w-full rounded-lg border px-3 py-2 text-sm outline-none" />{form.formState.errors.content && <p className="mt-1 text-xs text-red-600">{form.formState.errors.content.message}</p>}<div className="mt-3 flex justify-end"><button disabled={create.isPending || update.isPending} type="submit" className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700 dark:bg-slate-900 dark:hover:bg-slate-800 disabled:opacity-60"><Send size={15} />{editing ? 'Save comment' : 'Post comment'}</button></div></form>
    </section>
  );
}

...
const getStatusMutationError = (error: unknown): string => {
...
export default function TaskDetails() {
  ...
  const commentMutation = useMutation({
    mutationFn: (c: string) => createComment(id!, c),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ['comments', id] });
      void qc.invalidateQueries({ queryKey: ['activities', id, undefined] });
    }
  });
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: (updatedTask) => {
      qc.setQueryData(['task', id], (previousTask: any) => {
        if (!previousTask) return previousTask;
        return { ...previousTask, ...updatedTask, status: updatedTask.status };
      });
      setStatusMessage({ type: 'success', message: 'Task marked as completed.' });
      void qc.invalidateQueries({ queryKey: ['task', id] });
      void qc.invalidateQueries({ queryKey: ['activities', id, undefined] });
    },
    onError: (error) => {
      setStatusMessage({ type: 'error', message: getStatusMutationError(error) });
    }
  });
  ...
  return <div className="space-y-4">
    ...
    {statusMessage && (
      <div className={statusMessage.type === 'success' ? 'rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-700' : 'rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700'}>
        {statusMessage.message}
      </div>
    )}
    <Card><div className="flex flex-wrap gap-2"><Button onClick={() => handleStatusChange('TODO')} variant="secondary" disabled={statusMutation.isPending} loading={statusMutation.isPending && statusMutation.variables === 'TODO'}>To Do</Button><Button onClick={() => handleStatusChange('IN_PROGRESS')} variant="secondary" disabled={statusMutation.isPending} loading={statusMutation.isPending && statusMutation.variables === 'IN_PROGRESS'}>In Progress</Button><Button onClick={() => handleStatusChange('COMPLETED')} variant="primary" disabled={statusMutation.isPending} loading={statusMutation.isPending && statusMutation.variables === 'COMPLETED'}>Mark Completed</Button></div></Card>
